...
class SaveImage:
    ...
    def save_images(self, images, filename_prefix="ComfyUI", prompt=None, extra_pnginfo=None, context: execution_context.ExecutionContext = None, user_hash: str = ''):
        ...
        for (batch_number, image) in enumerate(images):
            ...
            metadata = None
            if not args.disable_metadata and not context.disable_pnginfo:
                metadata = PngInfo()
                if prompt is not None:
...
class LoadImageMask(LoadImage):
    ...
    def load_image_mask(self, image, channel, exec_context: execution_context.ExecutionContext):
        image_tensor, mask_tensor = super().load_image(image, exec_context)
        c = channel[0].upper()
    ...
    @classmethod
    def IS_CHANGED(s, image, channel, exec_context: execution_context.ExecutionContext):
        return super().IS_CHANGED(image, exec_context)
