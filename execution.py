...
def get_input_data(context: execution_context.ExecutionContext, inputs, class_def, unique_id, outputs=None, dynprompt=None, extra_data={}):
    ...
    if is_v3:
        if schema.hidden:
            ...
            if io.Hidden.context in schema.hidden:
                hidden_inputs_v3[io.Hidden.context] = context
...
async def execute(server, context: execution_context.ExecutionContext, dynprompt, caches, current_item, extra_data, executed, prompt_id, execution_list, pending_subgraph_results, pending_async_nodes):
    ...
    try:
        if unique_id in pending_async_nodes:
            ...
            input_data_all, missing_keys, hidden_inputs = get_input_data(context, inputs, class_def, unique_id, caches.outputs, dynprompt, extra_data)
            if server.client_id is not None:
    ...
    async def execute_async(self, context: execution_context.ExecutionContext, prompt, prompt_id, extra_data={}, execute_outputs=[]):
        ...
        with torch.inference_mode():
            ...
            while not execution_list.is_empty():
                ...
                result, error, ex = await execute(self.server, context, dynamic_prompt, self.caches, node_id, extra_data, executed, prompt_id, execution_list, pending_subgraph_results, pending_async_nodes)
