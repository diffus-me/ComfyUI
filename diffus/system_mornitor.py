    ...
    def wrapper(obj, input_data_all, extra_data, execution_block_cb=None, pre_execute_cb=None):
        ...
        with monitor_call_context(
            ...
            try:
                ...
                post_output_to_image_gallery(_make_headers(extra_data), input_data_all, output_data)
                return output_data
...
def _do_post_image_to_gallery(task_id, user_id, user_hash, image_type, image_subfolder, image_filename, pnginfo):
    ...
    post_json = {
        "task_id": task_id,
        "path": os.path.join("comfyui", folder_paths.get_relative_output_directory(user_hash), image_subfolder, image_filename),
        "feature": "COMFYUI",
        "pnginfo": pnginfo,
        "created_by": user_id,
        "base": None,
...
def post_output_to_image_gallery(header_dict, input_data, output_data):
    if not output_data:
        return
    ...
    _, ui_data, _ = output_data

    if not isinstance(ui_data, dict):
        return

    images_keys = ["images", "gifs", "audio", "video", "latents"]
    for images_key in images_keys:
