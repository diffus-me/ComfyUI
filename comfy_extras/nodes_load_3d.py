...
class Load3D(IO.ComfyNode):
    ...
    def execute(cls, model_file, image, **kwargs) -> IO.NodeOutput:
        ...
        output_image, ignore_mask = load_image_node.load_image(image=image['image'], user_hash=exec_context.user_hash)
        ignore_image, output_mask = load_image_node.load_image(image=image['mask'], user_hash=exec_context.user_hash)
        normal_image, ignore_mask2 = load_image_node.load_image(image=image['normal'], user_hash=exec_context.user_hash)
