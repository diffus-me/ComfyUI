...
class SaveAudioAdvanced(IO.ComfyNode):
    ...
    @classmethod
    def execute(cls, audio, filename_prefix: str, format: dict, exec_context: execution_context.ExecutionContext) -> IO.NodeOutput:
        file_format = format.get("format", None)
        quality = format.get("quality", None)
        if quality:
            ui=UI.AudioSaveHelper.get_save_audio_ui(exec_context, audio, filename_prefix=filename_prefix, cls=cls, format=file_format, quality=quality)
        else:
            ui=UI.AudioSaveHelper.get_save_audio_ui(exec_context, audio, filename_prefix=filename_prefix, cls=cls, format=file_format)
...
class PreviewAudio(IO.ComfyNode):
    ...
    def execute(cls, audio, exec_context: execution_context.ExecutionContext) -> IO.NodeOutput:
        ...
        return IO.NodeOutput(audio, ui=UI.PreviewAudio(audio, cls=cls, exec_context=exec_context))
